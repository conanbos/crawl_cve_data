...
def connecting():
    mydb = mysql.connector.connect(
        host="47.96.116.101",  # 数据库主机地址
        user="root",  # 数据库用户名
        passwd="Cherrytomato107",  # 数据库密码
        database="riskdb"
    )
    ...
    mycursor.execute("select * from risk_project")
    for x in mycursor:
        print(x)
...
class VData:
    ...
    def __init__(self):
        global cur
        global mysqldb
        mysqldb = self.connect()
        cur = mysqldb.cursor()
        # cur.execute("select * from risk_project")
        # for x in cur:
        #     print(x)
...
# drop table NVD_configuration;
# drop table nvd_cve;
# drop table nvd_description;
# drop table nvd_impact;
# drop table nvd_problemtype;
# drop table nvd;
# drop table nvd_refs;
